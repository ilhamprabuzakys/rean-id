<div>
    @if($posts->count() > 0)
    <h5 class="mb-4">Daftar Postingan</h5>
    @if($posts->count() > 0)
    <div class="p-3">
        {{ $posts->links() }}
    </div>
        ...
        <article wire:key="post-{{ $post->id }}"
            ...
            <div class="col-lg-5">
                <a href="{{ asset($post->files->first()->file_path ?? 'assets/landing/assan/assets/img/960x900/' . rand(1, 5) . '.jpg') }}"
                    class="d-block glightbox3 rounded-3 overflow-hidden hover-shadow-lg hover-lift" data-glightbox
                    data-gallery="gallery{{$key}}">
                    <img src="{{ asset($post->files->first()->file_path ?? 'assets/landing/assan/assets/img/960x900/' . rand(1, 5) . '.jpg') }}"
    ...
    @endif
    @else
    <h5 class="mb-4">Data Postingan masih kosong</h5>
    @endif
</div>